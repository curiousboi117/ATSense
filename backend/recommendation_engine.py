...
def generate_recommendations(
    ...
    if "skills" in sections:
        ...
        if "git" not in skills_text and "github" not in skills_text:
            rec = (
                "Add Git/GitHub to your Cloud/DevOps skills. Version control is a "
                "core requirement for almost all modern software engineering and "
                "data science roles."
            )
            if rec not in seen_recommendations:
                recommendations.append(rec)
    ...
    if "projects" in sections:
        ...
        if not any(
            kw in proj_text
            for kw in ["deploy", "aws", "docker", "gcp", "azure", "heroku", "vercel"]
        ):
            rec = (
                "Describe how your projects were deployed. Mentioning deployment "
                "tools (like Docker, AWS, or Vercel) demonstrates full-cycle "
                "software capability."
            )
            if rec not in seen_recommendations:
                recommendations.append(rec)
    ...
    if openai_key:
        enhanced = get_openai_suggestions(
            openai_key,
            text,
            job_description,
            recommendations,
        ...
        enhanced = get_gemini_suggestions(
            gemini_key,
            text,
            job_description,
            recommendations,
...
def get_openai_suggestions(
    ...
    prompt = (
        "You are an expert technical recruiter and ATS auditor. Analyze this resume "
        "text and target job description (if provided). "
        "Provide exactly 5 highly actionable, specific, and professional recommendations "
        "to improve this resume's ATS compatibility and content. "
        "Focus on phrasing, tech stack, metrics, structure, and action verbs. "
        "Make sure the advice is concrete, not generic.\n\n"
        f"Resume Text (Truncated):\n{resume_text[:2000]}\n\n"
        f"Job Description (Truncated):\n"
        f"{jd_text[:1500] if jd_text else 'None Provided'}\n\n"
        'Return a JSON array of strings, for example: '
        '["Recommendation 1", "Recommendation 2"]. '
        "Do not return markdown headers or formatting outside the JSON."
    )
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "You are a professional assistant that outputs raw JSON array of strings."
                ),
            },
            {
                "role": "user",
...
def get_gemini_suggestions(
    ...
    prompt = (
        ...
        'Return a JSON array of strings, for example: '
        '["Recommendation 1", "Recommendation 2"]. '
        "Do not return markdown block markers, just raw JSON."
    )
